---
title: Cenário — Disco não reconhecido
description: Procedimento completo para o cenário Disco não reconhecido - pré-requisitos, diagnóstico, correção, resultado esperado e riscos.
author: Edsilas
date: 2026-08-17
---
...
3. Testar noutra porta SATA.
4. Para M.2: verificar especificação do slot (SATA vs NVMe) no manual da placa.
5. `SE` disco com PCB queimada `ENTÃO`: substituir disco. Dados recuperáveis apenas via laboratório.
6. `SE` porta da placa-mãe defeituosa `ENTÃO`: usar outra porta ou adicionar controladora PCIe SATA.
...
- **Impacto no sistema:** Dados inacessíveis. SO não boota se for o disco de sistema.
